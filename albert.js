...
renderSuggestions();
updateSourceHub();
// Initialise les sessions au démarrage (crée Session 1 si nécessaire et persiste)
...
async function sendMessage() {
  ...
  if (dataExecution && shouldAnswerLocallyWithoutAlbert(dataExecution) && !isInfographicRequest(question)) {
    ...
    if (sDE && deCtx && deCtx.length > 20 && !_skipTools.includes(dataExecution.plan?.tool)) {
      ...
      } catch(e) { console.warn('[dataBlocks] Erreur:', e); }
    }
    return;
  }
...
${context || "(Aucun document chargé)"}`;
  ...
  try {
    ...
    // Enregistre la réponse Albert comme bloc composable pour le compositeur
    ...
    if (sAlbert && answer && answer.length > 80) {
      try {
        ...
        scheduleSessionsSave();
      } catch(e) { console.warn('[dataBlocks Albert] Erreur:', e); }
    }
