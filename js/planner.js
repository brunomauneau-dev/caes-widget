...
(function () {
  ...
  function columnKind(col) {
    ...
    if (/boursier/.test(c) && !/lycee|lyceen/.test(c)) return 'boursier';
  ...
  function contextScore(entry, q) {
    ...
    if (k === 'boursier' && /(boursier|boursiers|bourse)/.test(q)) s += 90;
  ...
  function addExplicitFilters(schema, q, filters, reasons) {
    ...
    // Boursiers avec négation.
    if (/boursier|boursiers|bourse/.test(q)) {
      const e = bestColumn(schema, x => x.kind === 'boursier', q);
      if (e) {
        const neg = /(exclu|exclure|excluant|hors|sans|non|pas).{0,30}(boursier|boursiers|bourse)|(boursier|boursiers).{0,25}(exclu|exclus|hors|non|sans|pas)/.test(q);
        const v = booleanValue(e, !neg) || findValue(e, [neg ? 'non' : 'oui']);
        if (v) upsertCandidateFilter(filters, { entry: e, item: v, alias: neg ? 'non boursier' : 'boursier', op: 'eq', score: 250 }, reasons);
      }
    }
  ...
  function detectTool(question) {
    ...
    if (/repartition|r[eé]partition|ventilation|par\s+|group[eé]|top|classement|pourcentage|proportion/.test(q) && !/combien|nombre|effectif/.test(q)) return 'group_by';
  ...
  window.buildPlannerPlan = function buildPlannerPlan(question, filterContextText, table, forcedTool = null) {
    ...
    const q = nrm(filterContextText || question);
    ...
    return {
      ...
      question,
      planner: {
        version: 'v18-planner-v2.0-schema-resolver',
        confidence,
        reasons,
        schema: {
