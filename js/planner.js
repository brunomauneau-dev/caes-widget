...
(function () {
  ...
  function addExplicitFilters(schema, q, filters, reasons) {
    ...
    if (/boursier|boursiers|bourse|crous/.test(q)) {
      ...
      if (e) {
        ...
        let v = booleanValue(e, !neg) || findValue(e, [neg ? 'non' : 'oui', neg ? 'false' : 'true', neg ? '0' : '1']);
        // Fallback si la colonne boursier contient des libellés inhabituels : prendre la première valeur oui/non détectable.
        if (!v && e.values && e.values.length) {
          const wanted = neg ? /^(non|no|false|faux|0)$/i : /^(oui|yes|true|vrai|1)$/i;
          v = e.values.find(x => wanted.test(String(x.value || '').trim()));
        }
        if (v) {
  ...
  window.buildPlannerPlan = function buildPlannerPlan(question, filterContextText, table, forcedTool = null) {
    ...
    return {
      ...
      question,
      planner: {
        version: 'v18-planner-v2.3-boursier-cache-proof',
        confidence,
        reasons,
        schema: {
