...
// V27.6 — évite que les follow-ups naturels du type "combien sont-ils ?"
// soient traités comme de nouvelles questions explicites. Une question de comptage
// ne repart de zéro que si elle introduit un concept absent du dernier plan.
function questionMentionsNewConceptAbsentFromPlan(question, prev) {
  if (!prev) return true;
  const q = normalizeText(question || '');
  if (!q) return false;
  ...
  const context = normalizeText([
    ...
    prev.targetCol2,
    ...(prev.mentionedCols || []),
    ...(prev.filters || []).flatMap(f => [f?.col, f?.value])
  ].filter(Boolean).join(' '));

  const concepts = [
    { q: /boursier|bourse/, ctx: /boursier|bourse/ },
    { q: /basque|pays basque/, ctx: /basque|pays basque|zone basque/ },
    { q: /academie|bordeaux|toulouse|poitiers|limoges|paris/, ctx: /academie|accueil|accept|bordeaux|toulouse|poitiers|limoges|paris/ },
    { q: /lycee|etablissement|scolarise|origine/, ctx: /lycee|etablissement|scolar|origine/ },
    { q: /serie|bac|general|technologique|professionnel/, ctx: /serie|bac|classe|general|technologique|professionnel/ },
    { q: /formation|filiere|mention|bts|but|dut|licence|cpge|prepa/, ctx: /formation|filiere|mention|groupe|bts|but|dut|licence|cpge|prepa/ },
    { q: /commune|ville/, ctx: /commune|ville/ },
    { q: /departement/, ctx: /departement/ },
    { q: /apprenti|apprentissage/, ctx: /apprenti|apprentissage/ },
    { q: /internat|interne/, ctx: /internat|interne/ }
  ];

  return concepts.some(c => c.q.test(q) && !c.ctx.test(context));
}


const DATA_ENGINE_CONCEPT_COLUMNS = {
  boursier: [
    { include: [/boursier|bourse/], exclude: [/non.*boursier.*groupe/], weight: 120 }
  ],
  basque: [
    { include: [/pays.*basque|basque.*pays|zone.*basque|basque/], exclude: [/commune|lycee|etablissement|formation/], weight: 120 }
  ],
  academie_accueil: [
    { include: [/academie/, /accueil|accept|affect|admission|formation/], exclude: [/origine|scolar|lycee/], weight: 130 },
    { include: [/academie/], exclude: [/origine|scolar|lycee/], weight: 70 }
  ],
  serie: [
    { include: [/serie/, /classe|bac/], exclude: [/formation|etablissement/], weight: 120 },
    { include: [/type.*bac|bac.*serie|bac/], exclude: [/formation|etablissement/], weight: 80 }
  ],
  formation_groupe: [
    { include: [/grands?.*groupes?.*formation|groupe.*formation|formation.*groupe/], exclude: [/etablissement|origine/], weight: 130 },
    { include: [/formation|filiere|specialite|mention/], exclude: [/etablissement|origine|commune|departement|academie/], weight: 70 }
  ],
  etablissement_accueil: [
    { include: [/etablissement|lycee|universite|iut|cfa|ecole/], exclude: [/origine|scolar/], weight: 100 },
    { include: [/accueil|accept|admission/], exclude: [/origine|commune|departement|academie/], weight: 45 }
  ],
  commune_origine: [
    { include: [/commune|ville/ , /origine|scolar|candidat/], exclude: [/accueil|accept/], weight: 120 }
  ],
  commune_accueil: [
    { include: [/commune|ville/, /accueil|accept|formation/], exclude: [/origine|scolar/], weight: 120 }
  ],
  departement_origine: [
    { include: [/departement/, /origine|scolar|candidat/], exclude: [/accueil|accept/], weight: 120 }
  ],
  apprenti: [
    { include: [/apprenti|apprentissage|contrat.*apprentissage/], exclude: [], weight: 120 }
  ],
  nb_voeux: [
    { include: [/voeu|voeux|vœu|vœux/, /confirm|class|nombre|nb/], exclude: [], weight: 120 }
  ],
  etablissement_origine: [
    { include: [/etablissement|lycee|lycee|uai/, /origine|scolar|provenance|candidat/], exclude: [/accueil|accept|admission/], weight: 150 },
    { include: [/lycee|etablissement/, /scolar/], exclude: [/accueil|accept|admission/], weight: 120 },
    { include: [/origine|provenance/], exclude: [/commune|departement|academie|accueil|accept/], weight: 80 }
  ],
  mention: [
    { include: [/mention|specialite|specialite/], exclude: [/etablissement|commune|departement|academie/], weight: 100 }
  ],
  type_contrat: [
    { include: [/type.*contrat|contrat/], exclude: [/date|debut|fin/], weight: 100 }
  ],
  internat: [
    { include: [/internat|interne/], exclude: [], weight: 100 }
  ]
};

function findColumnByConceptStrict(table, concept) {
  const headers = table?.headers || Object.keys(table?.objects?.[0] || {});
  const rules = DATA_ENGINE_CONCEPT_COLUMNS[concept] || [];
  if (!headers.length || !rules.length) return null;
  ...
  const scored = headers.map(h => {
    ...
    for (const rule of rules) {
      ...
      if (includes.every(re => re.test(hn)) && !excludes.some(re => re.test(hn))) {
        score += rule.weight || 1;
        // Bonus léger : colonnes métier spécifiques plutôt que libellés génériques.
        if (/origine|scolar|accueil|accept|formation|boursier|basque|academie/.test(hn)) score += 5;
        if (concept === 'etablissement_origine' && /origine/.test(hn)) score += 80;
        if (concept === 'etablissement_origine' && /scolar/.test(hn) && !/origine/.test(hn)) score -= 40;
      }
    }
    return { h, score };
...
function dataEngineTokenizeName(value) {
  const stop = new Set(['du','de','des','del','d','le','la','les','l','au','aux','a','en','et','sur','sous','st','ste','lycee','lycée','etablissement','etablissement','college','universite','uai']);
...
function fuzzyScoreValueAgainstQuery(value, query) {
  const vn = normalizeText(value || '');
  const qn = normalizeText(query || '');
  const tokens = dataEngineTokenizeName(query);
  const valueTokens = dataEngineTokenizeName(value);
  if (!vn || !tokens.length) return 0;
  let score = 0;
  let hits = 0;
  tokens.forEach(t => {
    if (vn.includes(t)) { hits += 1; score += t.length >= 5 ? 18 : 12; }
    else if (t.length >= 5 && vn.split(/\s+/).some(w => w.startsWith(t.slice(0, 5)) || t.startsWith(w.slice(0, 5)))) { hits += 1; score += 8; }
  });
  if (hits === tokens.length) score += 25;
  if (hits === tokens.length && valueTokens.length === tokens.length) score += 20;
  if (hits === tokens.length && valueTokens.length > tokens.length) score -= Math.min(12, (valueTokens.length - tokens.length) * 4);
  if (vn.includes(qn) || qn.includes(vn)) score += 30;
  score += Math.min(10, hits * 2);
  // Évite les matchs trop larges sur un seul mot court quand il y a plusieurs candidats.
  if (tokens.length === 1 && tokens[0].length < 5) score -= 20;
  return score;
}

function pickBestFuzzyColumnValue(table, col, query) {
  const vals = Array.from(new Set((table?.objects || [])
    .map(r => r?.[col])
    .filter(v => v !== undefined && v !== null && String(v).trim() !== '')
    .map(v => String(v).trim())));
  const scored = vals.map(v => ({ raw: v, score: fuzzyScoreValueAgainstQuery(v, query) }))
    .filter(x => x.score > 0)
    .sort((a,b) => b.score - a.score || String(a.raw).length - String(b.raw).length);
  if (!scored.length) return null;
  // Ambiguïté forte : deux valeurs quasi ex-aequo. On ne force pas un mauvais filtre.
  if (scored.length > 1 && scored[0].score - scored[1].score < 8) return null;
  return scored[0].raw;
}

function extractOriginSchoolQuery(question) {
  const raw = String(question || '').trim();
  const patterns = [
    /(?:lyc[eé]e|[eé]tablissement)\s+(?:d['’ ]?origine|de scolarit[eé]|scolaire)?\s*[:=]?\s*(.+?)(?:\?|$)/i,
    /(?:issus|issues|originaires|venant|provenant)\s+(?:du|de la|de l['’]|de|d['’])\s+(.+?)(?:\?|$)/i,
    /(?:qui viennent)\s+(?:du|de la|de l['’]|de|d['’])\s+(.+?)(?:\?|$)/i,
    /scolaris[eé]s?\s+(?:au|a la|à la|a l['’]|à l['’]|en)\s+(.+?)(?:\?|$)/i
  ];
  for (const re of patterns) {
    const m = raw.match(re);
    if (!m || !m[1]) continue;
    let name = m[1]
      .replace(/\b(?:combien|candidats?|eleves?|élèves?|sont|ils|elles|dans|sur|parcoursup)\b/ig, ' ')
      .replace(/[?.!]+$/g, '')
      .trim();
    if (dataEngineTokenizeName(name).length) return name;
  }
  return null;
}
...
function strictFiltersFromQuestion(table, question) {
  ...
  const originSchoolQuery = extractOriginSchoolQuery(question);
  if (originSchoolQuery) {
    const col = findColumnByConceptStrict(table, 'etablissement_origine');
    if (col) {
      const match = pickBestFuzzyColumnValue(table, col, originSchoolQuery);
      if (match) add(col, match, 'eq');
    }
  }
...
function inheritConversationContext(plan, question) {
  ...
  const introducesNewConcept = questionMentionsNewConceptAbsentFromPlan(question, prev);
  const explicitFresh = isExplicitFreshDataQuestion(question) && !/^(et pour|et les|idem|m[eê]me|meme)/.test(q) && introducesNewConcept;
  const implicitCountFollowUp = isExplicitFreshDataQuestion(question) && !introducesNewConcept;
  ...
  if (dimensionFollowUp || filterFollowUp || bareAction || implicitCountFollowUp || isFollowUpQuestion(question)) {
...
function inferMeasureIntent(question) {
  ...
  if (/combien|nombre|effectif|compte|compter|total|issus d[ue']|originaires? d[ue']|qui viennent|scolaris[eé]s? (?:au|a la|a l|en)|venant du|venant de|provenant du|provenant de|sont originaires/.test(q)) return 'count_rows';
...
function finalSanitizeAnalysisPlan(plan) {
  ...
  const introducesNewConcept = questionMentionsNewConceptAbsentFromPlan(plan.question || '', prev);
  const explicitFresh = isExplicitFreshDataQuestion(plan.question || '') && !/^(et pour|et les|idem|m[eê]me|meme)/.test(q) && introducesNewConcept;
  const implicitCountFollowUp = isExplicitFreshDataQuestion(plan.question || '') && !introducesNewConcept;
  ...
  if (prev && (dimensionFollowUp || filterFollowUp || implicitCountFollowUp)) {
...
function detectCompareGroups(table, question) {
  ...
  if (/basque/.test(q)) {
    const col = findColumnByConceptStrict(table, 'basque');
    if (col && /(non[- ]?basque|autres?|reste|hors|compare|versus)/.test(q)) {
  ...
  // Admis vs non-admis (réponse favorable)
  if (/(admis|admission|accept[eé]|répondu favorablement|avec proposition)/.test(q) &&
      /(non[- ]?admis|sans proposition|refus[eé]|pas accept[eé]|compare|versus)/.test(q)) {
    const col = (table.headers || []).find(h => /répondu favorablement|actuellement répondu/i.test(h));
    if (col) {
      addGroup('Admis',     [{ col, op: 'eq', value: 'Oui' }]);
      addGroup('Non admis', [{ col, op: 'eq', value: 'Non' }]);
      return groups;
    }
  }
  ...
  // ── 2. Détection générique : "compare X et Y" / "X vs Y" ────────────
  // Extraction des deux termes à comparer
  let termA = null, termB = null;
  const vsMatch =
    question.match(/(?:compare[rz]?\s+(?:les?\s+|la\s+|le\s+)?)(.+?)\s+(?:et|vs\.?|versus)\s+(?:les?\s+|la\s+|le\s+)?(.+?)(?:\s*[?!.]|$)/i) ||
    question.match(/(.+?)\s+vs\.?\s+(.+?)(?:\s*[?!.]|$)/i);
  if (vsMatch) { termA = vsMatch[1].trim(); termB = vsMatch[2].trim(); }

  if (termA && termB) {
    const nA = normalizeText(termA), nB = normalizeText(termB);
    const headers = table.headers || Object.keys(table?.objects?.[0] || {});
    // Cherche une colonne qui contient des valeurs proches de termA ET termB
    for (const col of headers) {
      const vals = [...new Set((table.objects || []).map(r => String(r[col] ?? '')).filter(Boolean))];
      if (vals.length < 2 || vals.length > 80) continue; // skip colonnes trop cardinalitaires
      const normVals = vals.map(v => ({ raw: v, n: normalizeText(v) }));
      const matchA = normVals.find(v => v.n.includes(nA) || nA.includes(v.n.slice(0, Math.min(v.n.length, 6))));
      const matchB = normVals.find(v => v.n.includes(nB) || nB.includes(v.n.slice(0, Math.min(v.n.length, 6))));
      if (matchA && matchB && matchA.raw !== matchB.raw) {
        addGroup(matchA.raw, [{ col, op: 'eq', value: matchA.raw }]);
        addGroup(matchB.raw, [{ col, op: 'eq', value: matchB.raw }]);
        return groups;
      }
    }

    // ── 2b. Comparaison de deux établissements d'origine ────────────
    const origCol = findColumnByConceptStrict(table, 'etablissement_origine');
    if (origCol) {
      const stopWords = new Set(['du','de','des','le','la','les','et','au','aux','en','lycee','lycée','etablissement']);
      const wordsA = nA.split(/\s+/).filter(w => w.length > 2 && !stopWords.has(w));
      const wordsB = nB.split(/\s+/).filter(w => w.length > 2 && !stopWords.has(w));
      const vals = [...new Set((table.objects || []).map(r => String(r[origCol] ?? '')).filter(Boolean))];
      const matchA = wordsA.length ? vals.find(v => wordsA.every(w => normalizeText(v).includes(w))) : null;
      const matchB = wordsB.length ? vals.find(v => wordsB.every(w => normalizeText(v).includes(w))) : null;
